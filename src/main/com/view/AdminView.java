	...
	private void manageMovies()
	{
		...
		while(choice!=-1)
		{
			if(choice==1)
			{
				...
				do
				{
					System.out.println("Enter movie cast and enter n/a to stop");

					 cast=sc.next();

				}
				while(!(cast.equals("n/a")));
				boolean remove = cast_members.remove("n/a");
				movie_manager.addMovie(title,duration, r2,s1,synopsis,director,cast_members);//ENUM
			}
			if(choice==2)
			{
				...
				String movie_title2 = sc.next();
				boolean b=false;
				for (Movie value : movieList) {
					if (movie_title2.equals(value.movie_title))
					{
			...
			if(choice==9)
			{
				...
				for (Movie value : movieList) {
					System.out.println(value.movie_title);
					System.out.println(value.durationMin);
					System.out.println(value.movie_director);
					System.out.println(value.rating);
					System.out.println(value.movie_synopsis);
					List <String> cast__members=value.cast;
					for(String cast_member:cast__members)
						System.out.print(cast_member+" , ");
					System.out.println();
					System.out.println(value.movieStatus);
